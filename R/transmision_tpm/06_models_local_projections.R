...
estimate_lp_product <- function(df, product_name, h_max = 12, shock_var = "dtpm") {
  ...
  controls <- available_controls_formula(dat0)
  ...
  purrr::map_dfr(0:h_max, function(h) {
    ...
    fml <- stats::as.formula(paste(
      "y_h ~", shock_var,
      "+ drate_l1 + dtpm_l1 + dtpm_l2 +", controls, "+ month_fe"
    ))
...
estimate_all_lp <- function(model_data, h_max = 12, shock_var = "dtpm") {
  ...
  purrr::map_dfr(products, ~ estimate_lp_product(model_data, .x, h_max = h_max, shock_var = shock_var))
}
