...
ipom_external |>
  ...
  summarise(
    ...
    `Desvío promedio vs baseline` = mean(difference_vs_baseline, na.rm = TRUE),
    `Desvío máximo vs baseline` = max(difference_vs_baseline, na.rm = TRUE),
    `Desvío mínimo vs baseline` = min(difference_vs_baseline, na.rm = TRUE),
    .groups = "drop"
  ) |>
  arrange(scenario, label) |>
  rename(Escenario = scenario, Variable = label, Unidad = unit) |>
  ipom_kable(caption = "Supuestos externos principales: niveles promedio y desvíos frente al baseline.")
```
